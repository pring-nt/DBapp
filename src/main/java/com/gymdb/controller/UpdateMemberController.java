...
public class UpdateMemberController {
    ...
    private void showEditDialog(Member member) {
        ...
        // NEW: Start BMI and Updated BMI fields
        TextField startBMIField = new TextField(member.startBMI() == null ? "" : String.valueOf(member.startBMI()));
        TextField updatedBMIField = new TextField(member.updatedBMI() == null ? "" : String.valueOf(member.updatedBMI()));
        ...
        grid.addRow(8, new Label("Start BMI:"), startBMIField);         // new row
        grid.addRow(9, new Label("Updated BMI:"), updatedBMIField);     // new row
        ...
        dialog.setResultConverter(dialogButton -> {
            if (dialogButton == saveButtonType) {
                ...
                // parse numeric weight and BMI fields (allow empty -> null)
                Double initW = null;
                Double goalW = null;
                Double startBMI = null;
                Double updatedBMI = null;
                try {
                    ...
                    if (!startBMIField.getText().trim().isEmpty()) {
                        startBMI = Double.parseDouble(startBMIField.getText().trim());
                    }
                    if (!updatedBMIField.getText().trim().isEmpty()) {
                        updatedBMI = Double.parseDouble(updatedBMIField.getText().trim());
                    }
                } catch (NumberFormatException ex) {
                    showAlert(Alert.AlertType.ERROR, "Initial/Goal weight and BMI must be numeric (e.g. 72.5 or 22.3).");
                    return null;
                }
                ...
                Member updated = new Member(
                        ...
                        initW,
                        goalW,
                        startBMI,       // NEW: set startBMI from field (or null)
                        updatedBMI,     // NEW: set updatedBMI from field (or null)
                        member.classID(),
                        member.trainerID(),
                        member.lockerID()
                );
